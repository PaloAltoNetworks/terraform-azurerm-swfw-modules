...
data "azurerm_public_ip_prefix" "allocate" {
  for_each = { for k, v in var.public_ips : k => v if v.create && v.prefix_name != null }
...
resource "azurerm_public_ip" "this" {
  for_each = { for k, v in var.public_ips : k => v if v.create }
...
locals {
  public_ips         = merge(azurerm_public_ip.this, data.azurerm_public_ip.this)
