...
variable "subnets" {
  ...
  - `default_outbound_access_enabled`      - (`bool`, optional, defaults to `false`) a flag that enables default outbound access
                                             to the Internet from the subnet. Using explicit Internet access methods is
                                             recommended.
  - `enable_storage_service_endpoint`      - (`bool`, optional, defaults to `false`) a flag that enables `Microsoft.Storage`
                                             service endpoint on a subnet. This is a suggested setting for the management
                                             interface when full bootstrapping using an Azure Storage Account is used.
  ...
  - `enable_private_link_network_policies` - (`bool`, optional, defaults to `true`) a flag that enables network policies for the 
                                             Private Link Service on the subnet.
  - `enable_appgw_delegation`              - (`bool`, optional, defaults to `false`) a flag that enables subnet delegation
                                             to `Microsoft.Network/applicationGateways` service. This is required for Application
  ...
  - `enable_cloudngfw_delegation`          - (`bool`, optional, defaults to `false`) a flag that enables subnet delegation to
                                             `PaloAltoNetworks.Cloudngfw/firewalls` service. This is required for Cloud NGFW to
                                             work in a VNET-based deployment.
  ...
  type = map(object({
    ...
    enable_private_link_network_policies = optional(bool, true)
