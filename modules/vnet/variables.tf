...
variable "subnets" {
  ...
  validation { # private_endpoint_network_policies
    condition = alltrue(flatten([
      for _, snet in var.subnets :
      contains(["Disabled", "Enabled", "NetworkSecurityGroupEnabled", "RouteTableEnabled"], snet.private_endpoint_network_policies)
      if snet.private_endpoint_network_policies != null
    ]))
    error_message = <<-EOF
    The `private_endpoint_network_policies` property can take one of the following values: 
