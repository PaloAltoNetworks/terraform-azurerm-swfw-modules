...
resource "azurerm_public_ip" "this" {
  count = try(var.natgw.create && var.public_ip.create, false) ? 1 : 0
  ...
  sku                 = "Standard"
  zones               = var.natgw.zone != null ? [var.natgw.zone] : null
...
data "azurerm_public_ip" "this" {
  count = try(var.natgw.create && !var.public_ip.create && var.public_ip.name != null, false) ? 1 : 0
...
resource "azurerm_public_ip_prefix" "this" {
  count = try(var.natgw.create && var.public_ip_prefix.create, false) ? 1 : 0
...
data "azurerm_public_ip_prefix" "this" {
  count = try(var.natgw.create && !var.public_ip_prefix.create && var.public_ip_prefix.name != null, false) ? 1 : 0
...
resource "azurerm_nat_gateway" "this" {
  count = var.natgw.create ? 1 : 0

  name                    = var.name
  resource_group_name     = var.resource_group_name
  location                = var.location
  sku_name                = "Standard"
  idle_timeout_in_minutes = var.natgw.idle_timeout
  zones                   = var.natgw.zone != null ? [var.natgw.zone] : null
...
locals {
  natgw_id = var.natgw.create ? azurerm_nat_gateway.this[0].id : data.azurerm_nat_gateway.this[0].id
...
resource "azurerm_nat_gateway_public_ip_association" "this" {
  count = var.natgw.create && var.public_ip != null ? 1 : 0
...
resource "azurerm_nat_gateway_public_ip_prefix_association" "nat_ips" {
  count = var.natgw.create && var.public_ip_prefix != null ? 1 : 0
