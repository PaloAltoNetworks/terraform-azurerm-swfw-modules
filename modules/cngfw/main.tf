...
resource "azurerm_palo_alto_next_generation_firewall_virtual_hub_panorama" "this" {
  ...
  network_profile {
    public_ip_address_ids        = [for v in var.cngfw_config.public_ip_keys : var.public_ip_ids[v]]
    egress_nat_ip_address_ids    = [for v in var.cngfw_config.egress_nat_ip_address_keys : var.public_ip_ids[v]]
    virtual_hub_id               = var.virtual_hub_id
  ...
  dynamic "destination_nat" {
    ...
    content {
      ...
      frontend_config {
        ...
        public_ip_address_id = var.public_ip_ids[destination_nat.value.frontend_public_ip_key].id
      }
      backend_config {
...
resource "azurerm_palo_alto_next_generation_firewall_virtual_network_panorama" "this" {
  ...
  network_profile {
    public_ip_address_ids     = [for v in var.cngfw_config.public_ip_keys : var.public_ip_ids[v]]
    egress_nat_ip_address_ids = try([for v in var.cngfw_config.egress_nat_ip_address_keys : var.public_ip_ids[v]])
    vnet_configuration {
  ...
  dynamic "destination_nat" {
    ...
    content {
      ...
      frontend_config {
        port                 = destination_nat.value.frontend_port
        public_ip_address_id = var.public_ip_ids[destination_nat.value.frontend_public_ip_key]
      }
      backend_config {
        port              = destination_nat.value.backend_port
        public_ip_address = destination_nat.value.backend_ip_address
