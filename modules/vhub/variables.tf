...
variable "virtual_wan_id" {
  description = "The ID of Virtual WAN"
  type        = string
}

variable "resource_group_name" {
  description = "The name of the Resource Group where the Virtual Hub should exist."
  type        = string
}

variable "region" {
  description = "The name of the Azure region to deploy virtual Hub"
  type        = string
  default     = null
}

variable "virtual_hub_address_prefix" {
  description = "The address prefix for the Virtual Hub. Must be a subnet no smaller than /24 (Microsoft recommends /23)"
  type        = string
  default     = null
}

variable "hub_routing_preference" {
  description = "Virtual Hub routing preference. Acceptable values are `ExpressRoute`, `ASPath` and `VpnGateway`."
  type        = string
  default     = "ExpressRoute"

  validation {
    condition     = contains(["ExpressRoute", "ASPath", "VpnGateway"], var.hub_routing_preference)
    error_message = "The Hub_routing_preference must be one of: ExpressRoute, ASPath, or VpnGateway."
  }
}
